  ...
  return (
    <nav style={s.nav}>
      ...
      <div style={s.links}>
        {user ? (
          <>
            <Link to="/marketplace" style={s.link}>
              Browse
            </Link>
            {user.role === 'farmer' && (
            ...
            {user.role === 'buyer' && (
              <Link to="/orders" style={s.link}>
                Orders
              </Link>
            )}
            {user.role === 'admin' && (
              <Link to="/admin" style={{ ...s.link, color: '#ffeaa7' }}>
                Admin
              </Link>
            )}
            {user.role !== 'admin' && (
              <Link to="/wallet" style={s.link}>
                Wallet
              </Link>
            )}
            <span style={{ color: '#d8f3dc', fontSize: 13 }}>
              {user.name} ({user.role})
            </span>
            <button
              style={s.toggleBtn}
