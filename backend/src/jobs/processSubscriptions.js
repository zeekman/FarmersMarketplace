  ...
  for (const sub of due) {
    ...
    try {
      ...
      db.transaction(() => {
        ...
        db.prepare('UPDATE subscriptions SET next_order_at = ? WHERE id = ?').run(nextOrderDate(sub.frequency), sub.id);
      })();

      console.log(`[subscriptions] Sub ${sub.id} → order ${orderId} paid (${txHash.slice(0, 12)}…)`);
    } catch (e) {
      // Payment failed — restore stock, pause subscription
      db.transaction(() => {
        db.prepare('UPDATE orders SET status = ? WHERE id = ?').run('failed', orderId);
        db.prepare('UPDATE products SET quantity = quantity + ? WHERE id = ?').run(sub.quantity, sub.product_id);
