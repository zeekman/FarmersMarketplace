...
router.get('/', (req, res) => {
  ...
  const total = db
    .prepare(`SELECT COUNT(*) as count FROM products p JOIN users u ON p.farmer_id = u.id ${where}`)
...
router.post('/', auth, validate.product, (req, res) => {
  ...
  const safeImageUrl =
    image_url && /^\/uploads\/[a-f0-9]+\.(jpg|jpeg|png|webp)$/i.test(image_url) ? image_url : null;
