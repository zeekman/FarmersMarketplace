...
function sendCsv(res, filename, columns, rows) {
  ...
  stringify(rows, { header: true, columns }, (e, output) => {
    if (e) return res.status(500).end('CSV error');
    res.end(output);
  });
}
...
router.get('/products/export', auth, farmerOnly, async (req, res) => {
  ...
  if (format === 'csv') return sendCsv(res, 'products.csv', columns, rows);
...
router.get('/orders/sales/export', auth, farmerOnly, async (req, res) => {
  ...
  let dateFilter = '';
  if (req.query.from) { dateFilter += ` AND o.created_at >= $${params.length + 1}`; params.push(req.query.from); }
  if (req.query.to)   { dateFilter += ` AND o.created_at <= $${params.length + 1}`; params.push(req.query.to + 'T23:59:59'); }
  ...
  if (format === 'csv') return sendCsv(res, 'sales.csv', columns, rows);
