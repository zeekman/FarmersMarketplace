...
router.post('/', auth, validate.review, async (req, res) => {
  if (req.user.role !== 'buyer') return err(res, 403, 'Only buyers can submit reviews', 'forbidden');
  ...
  if (!orderRows[0]) return err(res, 403, 'You can only review products from your paid orders', 'forbidden');

  const { rows: existing } = await db.query('SELECT id FROM reviews WHERE order_id = $1', [order_id]);
  if (existing[0]) return res.status(409).json({ success: false, message: 'You have already reviewed this order', code: 'duplicate_review' });
