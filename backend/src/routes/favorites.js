...
router.post('/', auth, async (req, res) => {
  ...
  try {
    await db.query('INSERT INTO favorites (buyer_id, product_id) VALUES ($1, $2)', [req.user.id, product_id]);
    res.json({ success: true, message: 'Added to favorites' });
  } catch (e) {
    if (e.message.includes('UNIQUE') || e.code === '23505') return err(res, 409, 'Already in favorites', 'already_favorited');
...
router.delete('/:product_id', auth, async (req, res) => {
  ...
  const { rowCount } = await db.query('DELETE FROM favorites WHERE buyer_id = $1 AND product_id = $2', [req.user.id, req.params.product_id]);
...
router.get('/', auth, async (req, res) => {
  ...
  const { rows: countRows } = await db.query('SELECT COUNT(*) as count FROM favorites WHERE buyer_id = $1', [req.user.id]);
...
router.get('/check/:product_id', auth, async (req, res) => {
  ...
  const { rows } = await db.query('SELECT id FROM favorites WHERE buyer_id = $1 AND product_id = $2', [req.user.id, req.params.product_id]);
