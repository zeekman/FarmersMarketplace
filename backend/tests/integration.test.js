...
const product = { id: 1, name: 'Organic Apples', price: 5.99, quantity: 10, farmer_id: 1, farmer_wallet: 'GPUB_FARMER', low_stock_threshold: 5, low_stock_alerted: 0 };
const buyer   = { id: 2, name: 'Buyer Bob', email: 'buyer@test.com', stellar_public_key: 'GPUB_BUYER', stellar_secret_key: 'SSECRET_BUYER', referred_by: null, referral_bonus_sent: 0 };
const farmer  = { id: 1, name: 'Farmer Alice', email: 'farmer@test.com', stellar_public_key: 'GPUB_FARMER' };
...
describe('Full User Flow: register → login → add product → create order → payment', () => {
  it('completes end-to-end flow successfully as farmer + buyer', async () => {
    ...
    const farmerReg = await request(app).post('/api/auth/register').send({ name: 'Farmer Alice', email: 'farmer@test.com', password: 'Secure1pass', role: 'farmer' });
    ...
    const buyerReg = await request(app).post('/api/auth/register').send({ name: 'Buyer Bob', email: 'buyer@test.com', password: 'Secure1pass', role: 'buyer' });
    ...
    const fundRes = await request(app).post('/api/wallet/fund').set('Authorization', `Bearer ${buyerToken}`).set('Cookie', cookie2).set('X-CSRF-Token', csrf2);
  ...
  it('fails order on insufficient stock, restores stock, marks failed', async () => {
    const buyerToken = require('jsonwebtoken').sign({ id: 2, role: 'buyer' }, process.env.JWT_SECRET || 'test-secret-for-jest');
